...
- **UpdateService** — the opt-in, off-by-default AppImage updater (FIBR-0054):
  ...
  API, verifies a downloaded release against its Ed25519 signature, and hands off
  the atomic swap + relaunch. Main responsibility: keep the app up to date without
  ever installing an unverified binary.
...
- **Tables (high-level):** `accounts`, `transactions`, `categories` (self-
  referential tree, `parent_id`), `categorization_rules`, `transfer_pairs` (records a
  confirmed or rejected transfer pair — success criterion 3, FIBR-0011), `import_profiles`, `statement_periods`
