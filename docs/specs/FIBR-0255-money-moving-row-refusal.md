...
set is the two tests that assert the new refusal** — INV-1's pair, the first of
which also carries INV-3's message assertions rather than having a test of its own.
...
| 3-impl | 2026-08-12 | **none — no reviewer was dispatched.** Written by `/write-spec` Step 8 from what building the contract found | 1 | 0 | 0 | 0 | 1 finding, fixed. Loop 3's own fix to §7 said the fail-first set was "the three tests … INV-1's two and INV-3's"; INV-3 has no test of its own — §10 maps it to INV-1's first test, whose message assertions are INV-3's. Running the suite made it visible immediately (2 red, not 3). Nothing else in the contract moved on contact: the two behavioural tests went red for the stated reason, the three non-regression tests were green before and after as §7 predicted, and INV-5's sweep came back byte-identical. |
