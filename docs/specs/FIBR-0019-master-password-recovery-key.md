...
**Scope: the conversion window.** The copy exists from S0 until S6 and is
then removed. It protects against the migration going wrong; it is not a
standing rollback for a downgrade after a migration that succeeded, and
§13.4 says so.
...
class Slot:
    nonce: bytes         # the 12 random bytes of §4.4's `nonce_hex`
    wrapped_dek: bytes   # §4.4's `wrapped_dek_hex` — ciphertext + GCM tag
...
`Slot` is the **ciphertext half only**. The slot's salt is NOT carried here: it
travels in the per-slot `KdfParams` (§4.4 — costs from the sidecar's `kdf`
group, salt from that slot's own record), which is what `derive_key` consumes
to produce the KEK. Spelled out because the v2 sidecar's record assembles from
this type, so leaving its shape to the implementer leaves that reader/writer
bound to the invention.
...
**What is fed to Argon2id is the DECODED value, never the text.** The 135-bit
payload as 17 fixed-width big-endian bytes, check symbol excluded. This is
not a detail: Crockford decodes `I` and `L` to `1` and `O` to `0`, so a
correctly-transcribed `AIB2…` and the printed `A1B2…` are the *same code* —
and deriving from the normalised string would give them different keys,
refusing a code the user copied correctly and throwing away the entire reason
§4.3 chose this alphabet. Generation, §4.6's unlock and INV-11's trial-unwrap
must all use this one input, or a slot written by one is unopenable by
another. It is also what makes D4's promise true: the display form can change
without invalidating a single code already written down.
...
sidecar version.** `models.FORMAT_VERSION` is the **shared** one:
`to_sidecar_dict()` stamps *it*, `BackupService` writes that same record as a
`.fbk`'s `params.json`, and the loader reads it back. Bumping
...
`format_version`; a v2 file carries none of the seven flat fields, so that gate
...
4. Generate the recovery code and derive **KEK-master**. KEK-recovery is
   NOT derived here: deriving it, wrapping the DEK under it and writing the
   slot all happen together at step 9, so Decline costs one Argon2id
   derivation and Keep costs two (§14, §13.5). Where a run does need both,
   they are **two sequential `DeriveWorker` runs**, not one widened run: that
   class takes a single password and emits a single key
   (`done = Signal(bytes)`), and widening its signal would drag every existing
   call site along with it.
5. Wrap the DEK into `slots.master`.
...
either way; deriving KEK-recovery, wrapping the DEK under it and writing the
slot all happen together at step 9 (§13.5), so Decline does none of them and
`slots.recovery` never reaches disk. The DEK, the master slot and the v2 sidecar are written
...
5. **Then offer to replace the recovery code** — D5's offer, for the user who
   came this way because they think their copy was exposed. Declinable, and
   declining keeps the existing code live; it is the same one-time display
   § 4.5 step 8 shows. Without this step the offer exists only as § 4.7's
   *Replace* in Settings, which § 15.3 records is not where users look.
...
- **INV-1** — **The installed vault's** SQLCipher raw key is a random DEK,
  ...
  *Test:* `tests/features/recovery_key/test_envelope.py::test_dek_is_not_derived_from_any_credential`
  — three legs. First, derive KEK-master from the master password and
  ...
  **differ**, which excludes any design deriving the DEK from the credential
  alone. Third, on a **migrated** vault, assert the **pre-migration v1 key no
  longer opens it**. The third leg is what reaches §8.1, and neither of the
  first two does: §8.1 is scoped to migrated vaults, and under it two vaults
  still hold different DEKs because their v1 salts differ — so leg 2 passes and
  the design this spec most wants to exclude ships green. §13.5 records the
  same thing from the other side. The first leg alone can only fail in the
  degenerate case where the KEK is itself the database key, and asking a
  migrated vault the same question is that leg again rather than a new one:
  what §8.1 makes permanent is the OLD key schedule, so the old key is what
  the leg must try.
...
- **INV-2** — Both slots unwrap to the same DEK, and either alone opens the
  ...
  *Test:* `tests/features/recovery_key/test_envelope.py::test_both_slots_yield_the_same_dek`
  ...
  and opens the vault twice, once per route. A third leg re-types the code in
  lower case with `I`, `l` and `O` substituted for `1`, `1` and `0`, and
  asserts it still opens — the transcription property §4.3 selects Crockford
  for, and the leg that fails if the KDF is fed the text instead of the
  decoded value. That substitution is a no-op on a generated code carrying
  neither a `1` nor a `0`, which is roughly one run in five, so the leg is
  paired with a FIXED-alphabet assertion that always bites:
  `decode("I" * 13 + "l" * 14 + "0") == decode("1" * 27 + "0")`.
...
- **INV-4** — The sidecar holds no unwrapped key material, no password and no
  ...
  *Test:* `tests/features/recovery_key/test_sidecar_v2.py::test_sidecar_holds_no_unwrapped_secret`
  ...
  vault created fresh** — a mid-migration one legally carries
  `migration_pending`, and any migrated one carries `cipher_compatibility`,
  both named in §4.4, and that no value anywhere in it contains the DEK,
...
- **INV-7** — Migration is atomic: at every instant, either the pre-migration
  pair or the post-migration pair opens with the user's password **and reads
  end to end, with every row intact**. There is no window where neither does.
  Opening alone is the weaker check and does not satisfy this: SQLCipher HMACs
  each page independently, so a file damaged past page 1 opens with its schema
  intact and its rows unreachable.
...
- **INV-12** — Declining a recovery key still builds the envelope.
  *Test:* `tests/features/recovery_key/test_sidecar_v2.py::test_declining_still_writes_the_envelope`
  — creates a vault while declining, asserts `sidecar_version == 2`, asserts
...
- **INV-13** — No byte of the live pair is modified until a rollback copy
  ...
  current key. Opening alone is the weaker check, for the reason INV-7 gives:
  a copy damaged past page 1 opens with its schema intact and its rows
  unreachable, which is worse than no copy — it is the artefact the user would
  be told to fall back on.
  *Test:* `tests/features/recovery_key/test_migration.py::test_no_swap_without_a_verified_rollback_copy`
  ...
  asserts in both cases that **the live sidecar is still the flat v1 record**,
  as well as that the vault still opens with the original password and that
  every table's row digest is unchanged. The sidecar leg is the one that can
  fail: under §13.1's inheritance the other two stay true across an S4 that ran
  with no verified copy, because the resume ladder opens the vault with the
  same password and changes no row — so a test built from those two alone is
  green over the exact breach this invariant names. **Not a hash of
  `vault.db`**, for the reason INV-12 gives: under WAL the main file's bytes
  move on open and close with no logical write, so a byte comparison is flaky
...
| The slot RECORD is well formed | A nonce, wrapped-DEK or salt of the wrong length in that slot | `KdfPolicyError` from `validate_slot`, refusing **that route only** and **without advancing the throttle** — "this record is damaged" is a different thing to say than "wrong credential", and `read_sidecar_v2` hard-fails on `master` alone precisely so a damaged optional slot does not bar the routes that still work. |
| The supplied credential unwraps its slot | Wrong password, wrong code, or a well-formed slot whose ciphertext does not authenticate | `KeyUnwrapError`, reported to the user as a failed attempt with no distinction between the causes (§4.2). Advance the shared throttle. |
| The DEK opens the database | Slot unwrapped but SQLCipher refuses | The pairing is broken — sidecar and database are from different vaults, or the database is damaged. Refuse, and point the user at restore-from-backup — **unless the sidecar says `migration_pending` and a verified `.pre-v2` pair is beside the vault**, in which case § 13.3's terminal branch offers the rollback instead, which is the whole return on D8. Do **not** offer the destructive reset from this state; it is indistinguishable to the user from a wrong password, and the consequences differ absolutely. |
...
| The rollback copy can be written and **read end to end** | Disk full, or the copy does not read end to end (`PRAGMA integrity_check`, not an open-probe — a copy damaged past page 1 opens with its schema intact and its rows unreachable) | Abort before S1. Nothing has been touched, so the vault is exactly as it was; report it and let the user free space and retry. **Never proceed without it** — that is INV-13, and a migration that skips its own safety net when the disk is tight is one that skips it exactly when it is most needed. |
...
Everything here changes **in the same release as the code**. A row still
outstanding asserts the opposite of what this spec builds; a row already
amended is kept **for the record, not as outstanding work** — `FIBR-0014.md`
D4, `security-model.md` INV-3 and INV-12 have landed, and re-applying one
leaves two amendment paragraphs a later reader has to reconcile.
...
| `docs/security-model.md` § 5 INV-12 · `services/auth.py::reset_vault` | INV-12 promises the destructive reset removes the vault's complete on-disk footprint, and §13 adds two more complete, still-openable copies to it: the `.pre-v2` pair and `vault.db.migrating`, each with its own WAL siblings. A `.pre-v2` pair opens under the password in force when the migration began, so a reset that leaves one hands the old vault to anyone holding a password the user has since changed. Both INV-12 and `reset_vault`'s enumeration need the artefacts. |
...
| `src/finbreak/vault.py` | `create` ends by calling `_write_sidecar(params)`, serialising the flat v1 object. §4.5 step 6 says to remove that write; **what shipped puts it behind `write_sidecar: bool = True`, with first-run passing `False` (§13.5)** — deleting it outright would break every v1 fixture §13's migration tests build from. `close` also needs the WAL siblings dealt with at §13's S5. |
...
| 4-amendment | 2026-08-20 | 3, cold — identical brief; re-arm loop for the D8 amendment, per CLAUDE.md rule 14 | 0 | 5 | 1 | 0 | **Six verified, six fixed; none dismissed. A VIOLENT ratio: five of the six landed on text written earlier in this same session** — three on loop 3's repairs, two on the D8 amendment itself, measured by `git log -S` on each anchor. **The run stops here and the document goes to implementation**; a fifth cold loop would mostly repair this loop's repairs, and there is nothing in the evidence to suggest it would stop. **All three lanes found the same two defects.** Loop 3 renamed the sidecar's version field to `sidecar_version` to break a collision with `validate_params`' first check, and INV-12 was left asserting `format_version == 2` — a key the file no longer carries, whose cheapest green is to stamp the very value the rename exists to prevent. And loop 3's `cipher_compatibility` field was given no position in §4.4's schema while INV-4 asserts the v2 shape *exactly*, so that test would have gone red on every migrated vault — i.e. on every vault in the field — with "stop recording the cipher level" as the plausible repair, which is the protection loop 3 added it for. **The amendment contradicted itself twice within the hour.** §13.4 promised a downgrading user "finds an intact `.pre-v2` pair waiting" while S6 deletes it on success, so the rollback covers the conversion window and nothing after it — now stated in D8's scope, §13.3's branch and §13.4 alike, rather than implied three different ways. And INV-13 asserted `vault.db` is byte-identical across the copy step, which is the exact witness INV-12 had ruled out two loops earlier under WAL. **The best non-collateral finding was two lanes on the same gap**: §4.1 wrote `derive_key(recovery_code, ...)` and the document never said whether the KDF eats the decoded value or the normalised text. Crockford decodes `I`/`L` to `1` and `O` to `0`, so text-derivation gives a correctly-transcribed code a different key and refuses it — discarding the entire reason §4.3 chose that alphabet, and silently falsifying D4's promise that the display form can change freely. Pinned to the 17-byte decoded payload, check symbol excluded, with a transcription leg added to INV-2. Also fixed: the "that constant is shared" antecedent pointed at the new constant rather than `FORMAT_VERSION`, and after the rename a v2 file lacks all seven v1 fields rather than six. |
...
| Step | Action |
|---|---|
| **S0** | Copy `vault.db` and `vault.kdf.json` to `vault.db.pre-v2` and `vault.kdf.json.pre-v2` (D8), **and any `-wal` / `-shm` sibling with them, after clearing the copy's own stale siblings first** — that ordering is what stops an earlier run's `-wal` being recovered into this fresh copy. `fsync` each copy and both parent directories, then **open the copy with the key already in hand and read it END TO END** (`PRAGMA integrity_check`, not an open-probe); abort the whole migration if either step fails. Nothing below runs until this succeeds. This is what INV-13 means by *complete*: a vault is four files (§6), so a two-file copy can open and still be missing the user's most recent rows. |
...
| **S4** | `os.replace` the sidecar, then **`fsync` its parent directory before S5 begins**. Both renames are atomic, but neither directory entry is durable until the parent is flushed — and if S5's reaches the platter while S4's does not, the user is left with a v1 sidecar over a DEK-keyed database, which every later unlock reports as a wrong password over an intact vault, and which § 13.3's ladder has no branch for (INV-7, FIBR-0327). |
...
| **S6** | Remove the `.pre-v2` pair (and its WAL siblings) **FIRST**, then rewrite the sidecar without `migration_pending` and `os.replace` it. **The order is what makes an interrupted S6 survivable**: `migration_pending` is the only thing that brings the ladder back here, so clearing it first and then failing to unlink — the disk-full and held-file class § 6 names — strands an encrypted copy that still opens under the master password of the moment, with no bookkeeping left to remove it and nothing that ever tries again. Failing the other way costs nothing: the flag stays set, the next unlock takes § 13.3 branch 1 and lands back here, where the unlinks are `missing_ok` and the write is idempotent. Removing the pair is safe because S2 verified the replacement row for row before anything was swapped, so past this point the copy protects nothing and is one more plaintext-adjacent artefact to look after. **This S6 does no fresh read, and § 13.3's does** — not an inconsistency: the straight-through path runs S2 to S6 in one uninterrupted call, so no crash can have arrived between them, which is exactly what the resume path cannot assume and why it re-reads. |
...
0. Derive KEK-master and unwrap `slots.master`. **The ladder is entered
   from that slot alone**: every branch below the first needs KEK-master,
   so another slot's KEK answers no to all of them and the rollback offer
   is withheld with nothing said (FIBR-0337 M1).
...
2. If `vault.db.migrating` exists, check it against the live vault.
   - **Reads end to end, and carries the live vault's row counts** → the crash
     ...
     (FIBR-0313 H1).
   - **Positive evidence about the replacement** — it does not read end to
     end, or both count sets were read and they differ → delete it as debris
     and continue; the live database answered, so step 3 restarts from it.
   - **Either file will not give up its counts** → nothing has been
     established about the replacement, so it is kept. Stated as a rule rather
     than as cases, because the case list left the replacement's own refusal
     in neither branch: a file that reads end to end and then declines its
     counts is a refusal to answer, not a short export. Delete only on
     positive evidence, and treat every unavailable count set as
     cannot-compare. Refuse here rather than continuing, because continuing
     cannot help — step 3 re-enters S1, which unlinks `vault.db.migrating`,
     and a restart re-reads the same live pages that have just refused. Offer
     the pre-upgrade pair on the terminal bullet's terms (FIBR-0337 H1).
3. Try KEK-master itself against `vault.db` — under §13.1 it is the v1
   database key, so no separate derivation is needed.
   - **Opens** → the crash was at or before S4. Secure the rollback copy
     first — the restart runs S4, which replaces the live sidecar, so INV-13
     binds here exactly as it does at S0 and carries no carve-out for the
     resume path. Reuse the S0 copy where it still verifies; otherwise retake
     it, **rebuilding the sidecar half from `slots.master`'s params rather
     than copying the live one**, because past S4 the live sidecar is the
     migration-pending v2 one and a byte copy would make "rollback" restore
     the stalled state. §13.1's inheritance is what makes the rebuild possible
     with nothing kept aside. Then restart from S1 **with the DEK unwrapped at
     step 0** — S1's "generate the DEK" is the first run through only, and a
     second DEK would leave the pending sidecar wrapping one no file answers
     to.
...
**A `.pre-v2` pair beside a *migration-pending* v2 sidecar is the rollback
route, and the app offers it rather than hiding it.** That is the only state
it exists in — S6 removes it — so this branch covers an interrupted
migration and nothing else (D8, §13.4). It is keyed by the password the user
...
and key record disagree" refusal. **The seam is named**, because both sides
bind to it: the ladder raises a distinct `RollbackAvailableError` (a
`VaultStateError` subclass, so an existing handler still catches it) and the UI
answers by calling `AuthService.restore_pre_upgrade_copy`. **That is the whole
return on D8: the
...
- **Backward** — a v2 vault cannot be opened by a build that predates this
  ...
  door **presented as data loss**. D8's rollback copy narrows this but does
  **not** close it: S6 removes the `.pre-v2` pair once the migration has
  completed, so it protects the conversion window and nothing after it. A
  user who downgrades following a *successful* migration has no `.pre-v2`
  route — their answer is a `.fbk` taken before upgrading, or staying on the
  new build. Stated rather than implied, because the opposite is the easy
  thing to assume.
...
- **§ 13.3's ladder is `vault_migration.resume`**, called from
  `AuthService._unlock_through_slot` *after* the **master** slot has
  unwrapped — which is § 13.3's step 0, and what keeps a mistyped password a failed attempt rather
...
under a *fresh* salt**, because that is random per vault — and § 8.1 is not
that case either: it keeps each vault's own v1 salt, which is also per-vault
random, so leg 2 passes there too. Leg 3 is what reaches § 8.1, and § 5 INV-1
...
- **Time, first run:** **Keep** costs two Argon2id derivations instead of one,
  so roughly double; **Decline** costs one, unchanged, because KEK-recovery is
  derived at step 9 and Decline never reaches it (§4.5, §13.5). Either way it
  is a one-time cost on a background worker (`DeriveWorker`).
...
2. **Should 1.0 refuse to ship while any supported path can still produce a v1
   ...
   build, a restore path missed in §11, or `Vault.create`'s `write_sidecar`
   keyword, which §13.5 records as shipping with a default of `True` and only
   first-run passing `False` — then "one key schedule" is an intention rather
   than a fact. The third is the one a reader will miss: it is live production
   code and it defaults on, so a new `create` call site writes a v1 sidecar
   unless its author knows to opt out.
