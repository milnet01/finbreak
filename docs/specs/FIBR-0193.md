...
5. **`ui/accounts.py::AccountsWidget._on_update` is updated *in this item*, to
   ...
   **The read goes through two new item-data roles, not a new service getter.**
   ...
   and each read back by some handler — `_on_update` reads the id role today,
   `_on_selection_changed` reads name and type, `_on_delete` and
   `_on_forget_password` read the name. This item adds
...
2. a vault at exactly v12 upgrades cleanly with its rows intact. This leg is the
   **only** observable for INV-1's central clause — leg 1 covers the *fresh*
   ...
   (ii) both columns are present and each one's `notnull` is `0` in
...
- **two** legs for INV-6 — new values for both fields, then `"   "` for both.
  ...
  fault localisation §5 argues for;
...
- **Carrying the Accounts-table UI here too.** Rejected: that fold-in is what
  took FIBR-0113 to 979 lines and five non-converging review loops. FIBR-0113
...
| §4.1 `vault.py::SCHEMA_VERSION` is not bumped **to `LATEST_SCHEMA_VERSION`** | `tests/features/accounts/test_migration_v13.py` leg 1 — a fresh vault reaching v13 *with both columns* is impossible if `create()` stamps 13 up front, because `run_migrations` would then iterate an empty range |
| §4.1 `vault.py::SCHEMA_VERSION` is not bumped **partially** (to any value 2–12) | **nothing** — `_migrate_to_v13` still runs, so leg 1 stays green while every earlier step is skipped. That state fails loudly *elsewhere* (a fresh vault missing `transactions`), but nothing in this spec observes it |
...
catcher anywhere in this project, and a partial `SCHEMA_VERSION` bump is caught
only by tests belonging to other features. The budget was three until the
`SCHEMA_VERSION` row was split: it named a catcher *and* conceded an uncaught
surface in one cell, which spec-format §5.8 forbids precisely because it hides a
`nothing` inside a row that reads as covered.
...
- `docs/specs/FIBR-0113.md` — **no edit needed, listed because it pins this
  ...
  pathologically long note cites §4.1, and its §4.1 cites **FIBR-0193 INV-4**.
...
- `docs/specs/FIBR-0172.md` — its **INV-14 row** pins `LATEST_SCHEMA_VERSION ==
  12`, and its **INV-14a row** goes stale in *two* places: the version pin to
  `12` **and** its `not in _MIGRATIONS` line, which advanced to `13` at v12 and
  must advance to `14` here. Updating only the version half leaves the row half
  stale. Its `### D5 — Dismissal persistence (schema v12)` and
