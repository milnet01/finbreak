...
it raises `TypeError` on every Update click — Python arity-checks at call time,
not at import, so this is a runtime break that mypy catches statically only
because that handler happens to be the one annotated caller (§11). It therefore
reads the selected account's current values and passes them straight back
(§3 decision 5).
...
5. **`ui/accounts.py::AccountsWidget._on_update` is updated *in this item*, to
   ...
   **The read goes through two new item-data roles, not a new service getter.**
   ...
   `_ACCOUNT_TYPE_ROLE` and `_ACCOUNT_HAS_PW_ROLE`, written in `_refresh` and
   read back by `_on_update` / `_on_selection_changed`. This item adds
   ...
   route reuses the pattern already carrying `name` and `type` to this exact
   call site and adds no public API that FIBR-0113 would then have to remove.
...
- **`reconciliation`'s *unregistered-next* guard stays green; its sibling and
  ...
  stale v13; all three are prose fixes (§12) and fall under the comment sweep. FIBR-0172's INV-14a is the precedent: it advanced the same guard's
...
2. a vault at exactly v12 upgrades cleanly with its rows intact — **accounts
   ...
   **Both named FK targets must actually have a row, or the check is vacuous.**
   ...
   them as `transactions` — two of its three existing call sites pass `[]` and
   seed nothing at all, so an implementer copying the idempotency precedent gets
   a `foreign_key_check` over an empty database that cannot fail. This leg
...
| §4.1 `vault.py::SCHEMA_VERSION` is not bumped **to `LATEST_SCHEMA_VERSION`** | `tests/features/accounts/test_migration_v13.py` leg 1 — a fresh vault reaching v13 *with both columns* is impossible if `create()` stamps 13 up front, because `run_migrations` would then iterate an empty range. **A partial bump is not caught here:** stamping any value between 2 and 12 still leaves `_migrate_to_v13` to run, so leg 1 stays green while every earlier step is skipped. That state fails loudly elsewhere (a fresh vault missing `transactions`), but nothing in *this* spec observes it |
...
- `docs/specs/FIBR-0113.md` — **no edit needed, listed because it pins this
  ...
  INV-10→**FIBR-0193 INV-5**. It also cites this spec by *section*: its §4.3
  cites "FIBR-0193 §4.2", its resource-cost section cites §4.1, and it cites
  **FIBR-0193 INV-4**. Those pointers are why INV-1..INV-5 here may never be
  renumbered (spec-format §3.7) **and why §4.1 / §4.2 may not be re-lettered**;
