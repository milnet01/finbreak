...
| § 2, backup row | `tests/features/backup/` INV-20 restores `.fbk` fixtures written by v0.1.12 and v0.1.22 through today's `restore_backup`. It samples those two schemas only, so a later schema needs a later fixture; `services/backup.py` refuses a *newer* schema, which is the other direction |
| § 2, export row | **nothing** — no test pins the PDF export's field set or layout across a version boundary |
| § 2, update path | `tests/features/auto_update/`, and `scripts/release-linux.sh`'s hard gate against the committed `RELEASE_PUBLIC_KEY_B64` — but both are **same-build**: the gate verifies against the key in the *same tree*, so rotating the key and the constant together passes it. `tests/features/auto_update/` INV-15 closes that: it verifies the shipped v0.1.23 `SHA256SUMS.sig` against the committed key, so a rotation turns it red. It tests the key, not the call path; re-signing its fixture to match a new key defeats it |
| § 2, saved import profiles | `tests/features/import_/` INV-12 restores a `.fbk` whose profile was saved by v0.1.12's own code, then matches it by header and parses a statement with it through today's build. It samples that one release only, so a later change to the profile table needs a later fixture |
