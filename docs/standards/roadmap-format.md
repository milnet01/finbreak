...
4. Tackle bullets in document order — *not* in ID order.
   **Store-backed (§3.0), read the severity prefix first**
   (§3.5.2 step 3), because a CRITICAL item appended last sits
   last. The prefix ranks bullets *within* step 3's selection —
   it does not override 🚧-first — and a bullet carrying none
   sorts after the prefixed ones, in document order.
