...
>
> **One CONTENT divergence is deliberate, and it is declared here so it
> is not "corrected" back.** The machine-wide
> `~/.claude/standards/changelog-format.md` writes the dated changelog heading with an em dash, `## [X.Y.Z] —
> YYYY-MM-DD`. This copy says **ASCII hyphen** (§4.1, §4.3), because
> that is what this project's `CHANGELOG.md`, its `.claude/bump.json`
> recipe and that recipe's `post_check` grep all use. The machine
> standard is the one that is wrong here; fixing it is its own change,
> under its own gate.
...
unavailable store-backed.** Where any other section of this
document teaches one — including the § 3 preamble above — read
it as hand-maintained unless it says otherwise:
...
is **emitted by the render**, so writing it by hand achieves
nothing and omitting it breaches nothing. §3.2's explicit
`<a name>` anchors are **neither emitted nor addable** — no
`roadmap_log` op writes one — so store-backed headings are
reachable only by the viewer's generated `roadmap-toc-N` names,
and §3.2's advice to embed one for a stable cross-reference does
not apply. Measured on this project's 7,567-line render: the
marker is on line 1, and there are zero explicit anchors.

Archive rotation is a third thing this section does not cover —
it is not specified in this copy at all; see the header note
above, which records that no store operation performs it today
either.
...
**Each of `Kind:`, `Source:` and `Lanes:` starts its own body
line.** That is what the render emits, and a one-pass parser
anchors on the line start; a field trailing prose on a shared
line is harvested by a mid-line scan and missed by every other
reader, so two counts of the same file disagree.
...
position in the file declares its priority — **hand-maintained
only (§3.0)**. When new items are
...
3. **Record the priority in the bold headline**, using §3.8's
   ...
   needs no field of its own. **Required inside a §3.8 findings
   fold-in, optional elsewhere**: store-backed it is the only
   priority signal available (§3.0), so an ordinary bullet
   without one is not lowest-priority but unranked — §3.5.4
   step 4 says where it sorts.
...
**A bullet with no `Kind:` is *unclassified*, and a consumer
must neither infer one from the section heading nor default it.**
A conforming file has none; this one does not conform yet (257
`Kind:` lines against 285 bullets), which is what the backfill
above is for. Stating the breach state is what lets two counts
of the same file agree meanwhile.
...
4. Tackle bullets in document order — *not* in ID order.
   **Store-backed (§3.0), rank step 3's selections by severity
   prefix** (§3.5.2 step 3) — it does not override 🚧-first, and
   a bullet carrying none sorts after the prefixed ones, in
   document order. Where no bullet carries a prefix, which is
   this project today, that reduces to document order.
...
1. `[Unreleased]` block contents move to a new dated section
   `## [X.Y.Z] - YYYY-MM-DD`. **An ASCII hyphen, not an em
   dash** — that is what Keep a Changelog uses, what
   `.claude/bump.json` writes, and what its `post_check` greps
   for, so a hand-authored em dash fails the release gate.
