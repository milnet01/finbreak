...
- **INV-3d — Slot wrapping is authenticated and fails closed**
  ...
  exactly, so it carries none of the freedom `memory_kib` has — a
  migrated vault legitimately keeps its own cost schedule, which is
  what the AAD exists to bind. The salt itself is bound through the
  derivation.
  **That safety rests on an obligation every route must keep, so it is
  stated here rather than assumed.** `read_sidecar_v2` hard-fails on
...
logging paths — § 5's *"With the phase that builds the code each governs"*
bullet enumerates which, and is the only list. The CI workflow
