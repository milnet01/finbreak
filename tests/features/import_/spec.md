...
| INV-5a | **The preview names the rows the dedup will drop** (FIBR-0204). `ImportPreview.duplicate_row_numbers` carries the `row_number` of every draft the multiset delta discards, and the wizard labels those rows **Duplicate** instead of **OK**. `duplicate_count` alone is a total with no way back to the rows, so the summary could read "1 new, 1 duplicate" while both rows showed OK — in a money app the row that is not going to land is the one worth pointing at. The kept set is already computed inside `_dedup`; membership is by object identity, since two equal drafts are exactly what a duplicate is. |
...
| INV-12 | **FIBR-0361.** A profile saved by an **earlier release's own code** still works in today's build. The fixture `tests/fixtures/backup_restore/v0.1.12-schema8-import-profile.fbk` was written by v0.1.12's `save_profile` + `export_backup` (provenance in that directory's `README.md`). Restored by today's `restore_backup`: `match_profile` on the same header returns the profile, its `column_mapping()` equals the mapping that was saved, and that mapping parses a debit/credit, day-first CSV into the right signed amounts and ISO dates. `test_INV12_profile_saved_by_an_earlier_release_still_matches`. |
