...
| INV-9 | **Schema v7 → v8.** `LATEST_SCHEMA_VERSION == 13`; a v7 vault upgrades to v8 with an empty `transfer_pairs` table, one atomic step (a wedged step leaves a re-openable v7). |
...
| INV-14 | **Bulk confirm / reject on a multi-select suggested table (FIBR-0201).** `transfers_suggested` is `MultiSelection`; `transfers_confirmed` stays `SingleSelection` (one `_make_table` builds both, so this is a parameter, not an edited line). `confirm_many(pairs)` confirms each in the **given order**, skipping any pair whose debit or credit an earlier confirm in the pass consumed — the earlier pair wins, the count is the number committed, and nothing raises at the user; `confirm_all()` *is* `confirm_many(candidate_pairs())`, so the consumed-set has one implementation. `reject_many(pairs)` records each (rejection consumes nothing, so two selected suggestions sharing a transaction are both recorded). Both slots resolve selected rows to domain ids **before** the first mutation, asserted under an applied sort that actually reorders the rows. The status line names any skipped suggestion rather than silently reporting a smaller number, and keeps `tr("Rejected.")` byte-for-byte at n == 1. |
