...
| INV-10 | **Schema v8 → v9.** `LATEST_SCHEMA_VERSION == 13`; a v8 vault upgrades with an empty `recurring_decisions` table, one atomic step (a wedged step leaves a re-openable v8); the `BackupService` version-ceiling gate rejects a v-too-new vault. |
