...
| INV-5 | **The transfer-candidate self-join seeks an index (FIBR-0327).** `transactions(amount_minor, occurred_on)`, added at v13 → v14: the v10 composite leads with `account_id`, so an equality on `amount_minor` alone cannot seek it, and SQLite fell back to building an AUTOMATIC index over the whole table on every call. `EXPLAIN QUERY PLAN` for `CANDIDATE_PAIRS_SQL` names the new index, contains no `AUTOMATIC`, and narrows on `occurred_on` as well as `amount_minor` — that last term is what tells a seek apart from reading every same-amount row, and it needs the date kept out of `julianday()`. |
