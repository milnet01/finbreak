...
| INV-4 | **When rules run.** Import categorises new rows in the same transaction; adding a rule does **not** re-file existing rows until `apply_rules()`. **Narrowed by FIBR-0213:** an import re-files only the rows *it* inserted, so "or the next import" no longer holds — a historical row matching a newly-added rule survives an unrelated import untouched, and the rows that import did insert are still categorised (both legs asserted, since scoping the recompute must not switch it off). |
| — | **Patterns are folded once per recompute, not once per row** (FIBR-0213). Both matchers `normalise_text`-folded every pattern inside the per-row loop — O(rows × patterns). Counted rather than timed (a wall-clock assertion is flaky on a shared machine, and the count is the property that changed): 12 rows × 8 patterns went from 120 folds to 20. Measured end to end at scale: 0.429s → 0.067s over 20k rows × 113 patterns. |
...
| INV-15 | **Schema v6 → v7** (this feature's delta). `LATEST_SCHEMA_VERSION == 14` now (later phases advanced it — v8 is FIBR-0011); a v6 vault walking to latest gains `transactions.category_id` + `category_source` + the `categorization_rules` table at the v6→v7 step, one atomic step (a wedged step leaves a re-openable v6). |
