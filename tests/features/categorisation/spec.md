...
| INV-15 | **Schema v6 → v7** (this feature's delta). `LATEST_SCHEMA_VERSION == 11` now (later phases advanced it — v8 is FIBR-0011); a v6 vault walking to latest gains `transactions.category_id` + `category_source` + the `categorization_rules` table at the v6→v7 step, one atomic step (a wedged step leaves a re-openable v6). |
