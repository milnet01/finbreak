...
| INV-15 | **Schema v6 → v7.** `LATEST_SCHEMA_VERSION == 7`; a v6 vault upgrades to v7 with `transactions.category_id` + `category_source` + the `categorization_rules` table, one atomic step (a wedged step leaves a re-openable v6). |
