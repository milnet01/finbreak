...
| INV-10 | `test_INV10_deb_rules_commands_are_valid_shell` | every command in `debian/rules` parses under `sh -n`, split the way `make` runs them — one invocation per non-continued line, backslash continuations joined. Nothing else in the gate reads `debian/rules` (`shellcheck` does not take a Makefile), so the first reader of a syntax error is an OBS build root |
