...
| INV-18 | **FIBR-0313 H4.** The interrupted-restore recovery path carries the original's WAL siblings BACK, on the same terms `_install` carries them aside (FIBR-0310 P5 fixed only the aside half). Without it the recovered vault silently loses any committed-but-not-checkpointed tail — the user's most recent transactions. Both branches are asserted: where the original HAS a journal it arrives with its own bytes, and where it has none the aborted restore's orphan is REMOVED, since that journal belongs to a different database under a different key. The fix predates this invariant — it landed under FIBR-0318 — and `mutation_probe` showed nothing tested it: removing the move-back left the whole backup suite green. | `test_interrupted_restore_recovers_the_wal_siblings_too`, `test_interrupted_restore_clears_an_orphan_wal_the_original_lacked` |
| INV-19 | **FIBR-0314.** The interrupted-restore recovery FLUSHES the directory it renamed into. `os.replace` is atomic but its directory entry is not durable until the parent is flushed, so a power loss just after the only recovery path that exists could revert it. Same rule as `backup._install` and `vault_migration.restore_rollback_copy`; this was the last site in that family, and the hand-off the bullet expected never happened — FIBR-0313 H4 landed as FIBR-0318 and removed the unlink-first window without adding the flush. The leg records the real `os.fsync`, filters to this vault's directory by `(st_dev, st_ino)` so an unrelated startup flush cannot satisfy it, and asserts the flush comes AFTER the last rename, since flushing before it commits nothing. | `test_the_recovered_original_pair_is_made_durable` |
| INV-20 | **FIBR-0302.** `restore_backup` restores a `.fbk` written by an **earlier release's own code** — every prior test in this suite exports and restores within the same build, so nothing had ever crossed a version boundary despite `versioning.md` § 2 naming exactly this "a backup taken on any earlier release cannot be restored" as a MAJOR break. Two fixtures, each a real `.fbk` produced by that release's own `BackupService.export_backup` under `tests/fixtures/backup_restore/` (provenance + regeneration in that directory's `README.md`): one from **v0.1.22** (schema 13, the last release before FIBR-0019's key envelope), one from **v0.1.12** (schema 8, several migrations further back). Restoring either through **today's** `restore_backup` must: succeed; open under the credential restore sets up (the new master password, never the fixture's original); reproduce the two synthetic seeded transactions; migrate to `LATEST_SCHEMA_VERSION`; and — the FIBR-0019 half of this invariant — install a **v2** sidecar (`sidecar_version() == SIDECAR_VERSION`), proving a restore of a backup that predates the key envelope still lands on the new sidecar shape rather than silently keeping the old flat v1 one. | `test_INV20_restores_a_backup_from_an_earlier_release[v0_1_22]`, `test_INV20_restores_a_backup_from_an_earlier_release[v0_1_12]` |
