...
| INV-8 | **No schema change of its own** — reconciliation registers no migration. The schema has since advanced to `LATEST_SCHEMA_VERSION == 12` (via FIBR-0172's `alert_dismissals`, unrelated to reconciliation), so the guard now pins the current latest and that no *unregistered next* version (v13) exists. | `test_no_schema_change.py` |
