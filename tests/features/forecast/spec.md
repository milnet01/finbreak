...
| INV-9 | **Migration v10 → v11.** Additive nullable `ALTER TABLE ADD COLUMN`, version-gated, old rows NULL; the v11 step adds the column and `run_migrations` then walks the vault on to the current latest, idempotent, atomic rollback on a wedged ALTER. | `test_migration_v11.py` |
