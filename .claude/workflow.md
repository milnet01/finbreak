...
> **CURRENT (2026-07-20, autonomous "build it all" run) — trust this + ROADMAP over the stale table below.**
> Just shipped (local, unpushed): **FIBR-0029** ✅ (password hint before unlock) — optional plaintext hint on the unlock screen behind a reveal-on-click "Show hint" button; SET from Settings with a current-password confirm (new `AuthService.verify_password`, constant-time); enforced never to equal/contain the master password (`services/password_hint.validate_hint`, NFC+casefold, unconditional containment); stored in plaintext `window.ini`. `security-model.md` A1 + INV-11 (cold-eyes converged loop 1). Spec cold-eyes converged loop 2 (loop 1 caught a short-password leak). 24 tests; full gate green (1197 passed, 2 skipped). commit 4ddf725, **tag FIBR-0029-complete**. Prior this session (all pushed): **FIBR-0153** ✅ (currency display), **FIBR-0032** ✅ (clipboard copy + auto-clear), **FIBR-0151** + **FIBR-0033**. Still 📋: **FIBR-0154** (3rd-tier category UI).
> Tree clean, `main` level with origin (FIBR-0029 batch pushed). **NEXT: FIBR-0030** (forgotten-password destructive reset) — user-chosen 2026-07-20; **design + recon DONE** (durable in the FIBR-0030 ROADMAP annotation: new "Start over" button on UnlockDialog → shell-owned double-confirm (warning + type DELETE) → new `AuthService.reset_vault()` deletes vault.db + sidecar **+ the WAL sidecars vault.db-wal/-shm** + clears throttle/hint window.ini keys → `_route_pre_login()` back to first-run). **NEXT STEP: write `docs/specs/FIBR-0030.md` → /cold-eyes --max-loops 7 → reproduce-first TDD → close.** Then **FIBR-0096** (SHA256SUMS + SBOM) or **FIBR-0154** (3rd-tier categories).
