...
| **Active step** | 3 (spec written + `/cold-eyes` **converged at loop 3** 2026-07-09 — 0 CRIT/0 HIGH, cleared for code. Scope EXPANDED from the approved Round-1 design by two same-day follow-ups: exact per-statement txn count + delete-statement-with-transactions → pulls the statement-provenance stamp (v5→v6 migration) into this phase. Next: TDD — write `tests/features/statements/` + the FIBR-0051 `app_shell` ripple + the v5→v6 schema ripple across vault/accounts/categories/import_/ofx_import/pdf_import, then implement to green → `/close-phase`.) |
| **Blocked on** | — |
...
3. 🚧 Write failing tests (`tests/features/statements/` + the FIBR-0051 app_shell ripple + the v5→v6 schema ripple across 6 suites)
