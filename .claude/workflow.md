...
| **Active item ID** | **FIBR-0065** (auto-lock-during-modal-dialog crash fix — the reproduced HIGH from the 2026-07-10 sweep, pulled ahead of FIBR-0054 per the user 2026-07-10). Approach chosen with the user: convert the 6 remaining blocking content-widget `exec()` pop-ups to the shell's non-blocking `setModal(True)`+`show()`+signal pattern (FIBR-0051 D2). Step 1 in progress: `docs/specs/FIBR-0065.md` drafted, `/cold-eyes` running (3 cold lanes). — prior active item: FIBR-0054 (in-app auto-update — NOT yet started). The remaining near-term user ask; order was the session's call, "it all ships anyway". Deps: none blocking. Next: brainstorm → spec → `/cold-eyes` → TDD → `/close-phase`. (After it: P09/FIBR-0011 transfer detection, then P10/FIBR-0012 the Home spending-by-category dashboard, which now has the category link FIBR-0010 created + still awaits FIBR-0011.) |
