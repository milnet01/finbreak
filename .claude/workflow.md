...
> **NEXT STEP — FIBR-0155** (OBS / openSUSE Build Service native RPM/deb publishing; user has a build.opensuse.org account). Needs: **spec** (`docs/specs/FIBR-0155.md`) → **/cold-eyes --max-loops 7** (this IS a multi-file design doc — full cold-eyes, not a self-read) → implement (OBS project config + RPM `.spec` + `debian/` recipe + .desktop/icon/AppStream metainfo mirroring CHANGELOG) → verify a real OBS build. Key design decision for the spec: **native-from-source packaging vs wrapping the frozen AppImage** (native = the app-store experience OBS is built for) + which distro targets to enable. Then the **Ants MCP feedback pass** to close the run. **The FIBR-0154 API surface delivered (for reference):**
