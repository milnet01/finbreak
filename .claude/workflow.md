...
> **IN PROGRESS — FIBR-0096** (per-release SHA256SUMS + SBOM). Turned out **not** mechanical: recon found publishing is two separate manual host-side runs (`release-linux.sh` then `release-windows.sh`), so `SHA256SUMS` must be **two-phase** (phase-1 AppImage line → phase-2 merge in the exe, re-sign, re-upload); the bundled dep tree only exists in the build container (no lockfile) so the SBOM must be generated in-build. **User decisions (2026-07-21):** coverage = **Linux AppImage + Windows exe both**; SBOM tool = **reuse `pip-audit`** (CycloneDX, no new dep). **Spec `docs/specs/FIBR-0096.md` DRAFTED + committed** (baseline, **unpushed**). **NEXT STEP: `/cold-eyes docs/specs/FIBR-0096.md --max-loops 7` → reproduce-first TDD → close.** (Decisions are baked into the spec's Status header + §2, so they survive a compact.) Then **FIBR-0154** (3rd-tier category UI), then an Ants MCP feedback pass to close the run.
> Unpushed: the FIBR-0096 spec baseline + this workflow update (2 docs commits — safe in git, push on resume).
