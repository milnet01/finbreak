...
> **NEXT — FIBR-0154** (3rd-tier category UI): data model already supports 3-tier (FIBR-0006); the gap is **UI-only**. Needs spec → `/cold-eyes --max-loops 7` → reproduce-first TDD → close. Then an **Ants MCP feedback pass** (mark completed fixes in `/mnt/Games/Scripts/Linux/finbreak_Ants_MCP_Feedback.md`, don't overwrite maintainer roadmap-ID annotations) to close the run.
