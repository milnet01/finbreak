...
| **Push held** | 2026-08-07: **the commit count is deliberately NOT recorded here** — every attempt to keep one current went stale the instant the commit that wrote it landed (this row has said 20, 25, 27 and 28, each arithmetic on top of an already-stale figure). **Measure it: `git rev-list --count @{u}..HEAD`.** Unpushed tags are exactly two — `FIBR-0085-complete` and `FIBR-0252-complete` — confirmed by diffing `git tag` against `git ls-remote --tags origin`; note `git for-each-ref --no-contains @{u} refs/tags` does NOT give an unpushed-tag count (it lists nearly every tag). The batch is the FIBR-0085 commits + the FIBR-0252 spec/review commits + FIBR-0252's implementation and close. Held ONLY by the GitHub Actions outage, not by anything wrong with them: last checked 2026-08-06 21:30 UTC — `major_outage`, incident still "investigating", Pages down too; Git Operations, API and Webhooks operational. **Re-check before pushing** — this row is a snapshot, not a live reading. Gate green locally at the last run: 1867 passed / 3 skipped, mypy clean over 183 files. **One** red CI run on `983559e` (`31117840312`, 15m35s vs a normal ~3min, died before any stage ran) — the outage, not a defect. When Actions is healthy: activate `.venv`, `git push origin main`, then `git push --tags --no-verify`, re-run `31117840312`, and clear this row. |
