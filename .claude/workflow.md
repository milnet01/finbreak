...
> Just shipped (local, unpushed): **FIBR-0153** ✅ (currency display) — `_format_amount` maps the base-currency ISO code to a symbol via a single-homed `CURRENCY_SYMBOLS` map and composes "`R 1,234.49`" (symbol + one space, no ISO code) via `QLocale().toString(x,"f",d)` — fixing every money site app-wide (Transactions, Home, PDF). New dedicated **Currency** column on the Transactions table (`_COL_CURRENCY=2`, shifting Description/Account/Category → 3/4/5). Spec cold-eyes **converged loop 5** — loop 2 empirically caught a CRITICAL: `toCurrencyString(v,"")` leaks the ISO code in non-C locales; switched to `toString`. INV-1..9 tested (en_US + en_ZA legs; stale 5-col layout safe). Full gate green (1173 passed, 2 skipped). commit 4132649, **tag FIBR-0153-complete**. Earlier this session: **FIBR-0032** ✅ (clipboard copy + auto-clear, tag FIBR-0032-complete, pushed), **FIBR-0151** + **FIBR-0033** (pushed). Still 📋: **FIBR-0154** (3rd-tier category UI).
> **UNPUSHED BATCH queued** — FIBR-0153 spec (cold-eyes loops 1–5) + impl + close-out. Push the batch next. **NEXT after push: FIBR-0029** (password hint before unlock), **FIBR-0030** (forgotten-password destructive reset), **FIBR-0096** (per-release SHA256SUMS + SBOM), or **FIBR-0154** (3rd-tier categories) from the backlog.
