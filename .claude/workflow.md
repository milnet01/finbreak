...
**This header holds only facts that live nowhere else.** Everything
that *can* be derived is derived, on purpose — the previous version of
this section was a 128 KB hand-written narrative that drifted weeks out
...
(CLAUDE.md § Where state lives item 2 owns this rule, FIBR-0281.) **§1 does
still own which ONE open item is active, and its step** — the DB records
statuses, not which 🚧 item is being worked. `CHANGELOG.md` is the
...
These four are configuration and state that no other file records.
...
Maintained by `/close-phase` (which sets the item, resets the step
checkboxes, and bumps the date on every close). **Work done outside the
phase loop — self-directed fixes, debt items, releases — does not touch
this**, so treat a stale date as "recent work was out-of-band", not as
"nothing has happened".

| Field | Value |
|---|---|
| **Active item ID** | None. FP05 / FIBR-0337 closed 2026-09-06, and with it the FIBR-0019 chain. |
| **Blocked on** | Nothing. FIBR-0019 is ✅ and the four fix-passes it spawned (FP02 FIBR-0307, FP03 FIBR-0310, FP04 FIBR-0313, FP05 FIBR-0337) are all ✅. **0.1.23 is unblocked**: the CHANGELOG `[Unreleased]` section claims FIBR-0019 shipped and it now has. FIBR-0159 and FIBR-0133 stay 🚧 (packaging) and neither is a 1.0 blocker. The resume ladder's recurring defect shape is FIBR-0339, filed rather than made into FP06. |
| **Last update** | 2026-09-06 (FP05 closed and the FIBR-0019 chain discharged. Every FP05 finding fixed and gate-green; two contract documents were amended, so rule 14's gate ran on each to this project's cap and both capped calm. `/close-phase` was deliberately not run — see the step note below.) |

### Step progress

While an item is active, Claude marks the current step 🚧; completed
...
5. ⬜ Run `check-code` — in parallel with 6 — **deliberately not run**
6. ⬜ Run `review-code` — in parallel with 5 — **deliberately not run**
7. ⬜ Fold all actionable findings into one new fix-pass — **not run**
8. ✅ Update CHANGELOG, flip status, write journal
9. ✅ Commit, tag `<ID>-complete`, update this file, ask about push

**Steps 5–7 were deliberately not run for FP05.** They are
`/close-phase`'s, and they would have produced FP06 — the fifth consecutive
fix-pass, which trips the convergence checkpoint above. The checkpoint exists
to force that call, so the recurring defect shape is filed as **FIBR-0339**
instead. What was skipped is the search for another round, not any of the
work: FP05's own findings were all fixed and gate-green first.

**One item is code-complete but not formally closed: FIBR-0054**
(auto-update) sits at step 4 ✅, steps 5–9 ⬜, waiting on a live
user-run update test. Its full history is in the §3 journal and its
caveat — the *running* (old) version performs each relaunch, so a
relaunch fix only proves out on the update *after* the one that ships
it — is recorded in `docs/specs/FIBR-0054.md`.

### Active item details

Filled in by `/close-phase` when it hands over to the next item; the
per-item contract itself lives in `docs/specs/<ID>.md`.

**FP02 runs finding-by-finding, so steps 3 and 4 interleave** — each
finding gets its failing test proved red, then its fix, then the suite.
That is why both are 🚧 rather than 3 ✅ / 4 🚧. Steps 1 and 2 are ✅ for
the pass as a whole: the FIBR-0307 bullet already names, per finding, the
...
whose wording describes this exact shape — an API returns immutable
`bytes`, they are copied into a wipeable buffer and the original
reference dropped. FP02 **documents** it in `keywrap.py` rather than
fixing it; do not re-open it. Still open and NOT blocking: spec § 15's
three release-time questions.

**FP02's close was ATTEMPTED on 2026-08-25 and BLOCKED.** `check-code` was
clean; `review-code`'s four lanes found nine defects FP02 itself introduced
plus twelve pre-existing, folded into FP03 (FIBR-0310). The close record is
`docs/journal/FIBR-0307.md`. **The one to know without reading it: FP02's
clipboard fix does not work** — the auto-clear guard is parented to the
dialog, so the timer dies with it, and the test passed only because it
called `clear_if_ours()` directly instead of letting the timer fire.

**FP03's close was ATTEMPTED on 2026-08-25 and BLOCKED**, from a cleared
context, which is what FIBR-0310 required. `check-code` was clean on the
scope; `review-code`'s four lanes returned one CRITICAL, four HIGH after
calibration, and a long tail, folded into FP04 (FIBR-0313). The close
record is `docs/journal/FIBR-0310.md`. **The two to know without reading
it.** The CRITICAL: a vault that opens but does not read end-to-end takes
branch 1 of `resume` on every later unlock, so `RollbackAvailableError` is
never raised and the user is stranded with unreachable rows beside a
verified `.pre-v2` pair nothing will mention. And the pattern three lanes
found independently: **an FP03 fix that reached some call sites and not
all** — R5, P7 and P5 each landed where the finding pointed, and nothing
asked where else the same rule binds.

**Do not flip FIBR-0019 to ✅ until FP02 closes clean** — the 🚧 is a
deliberate interlock, because `cut-release` refuses when a CHANGELOG
section claims an id that is not ✅ and the FIBR-0019 entry is already in
`[Unreleased]`. No CHANGELOG entry exists for FP02 on purpose: it would
claim a fix that has not happened.
