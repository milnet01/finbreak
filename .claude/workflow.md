...
> Just shipped (local, unpushed): **FIBR-0030** ✅ (forgotten-password destructive "start over" reset) — flat "Forgot password? Start over…" button on the unlock screen → shell-owned double-confirm (Step-1 warning, Cancel default; Step-2 `StartOverDialog` type-`DELETE` gate, `exec()`-driven so it wires `accepted→accept` itself) → new `AuthService.reset_vault()` deletes the **complete** on-disk footprint (vault.db + KDF sidecar + **both SQLite WAL sidecars vault.db-wal/-shm** — nothing else cleaned them) → on success clears throttle+hint `window.ini` keys → teardown → `_route_pre_login()` back to first-run. `OSError`→`QMessageBox.critical`, stay on unlock (nothing over-promised). `security-model.md` INV-12 (deletion-completeness *hygiene*, logical delete ≠ secure wipe) + T11 last-resort note (cold-eyes clean, 1 LOW fixed). **Spec cold-eyes CONVERGED loop 6** of the 7-loop cap (caught genuine spec bugs each loop: vacuous/self-contradicting tests, a dead-on-arrival OK button, a T1 mis-citation). Reproduce-first TDD, 12 vault_reset tests; full gate green (**1209 passed**, 2 skipped). commit 16de444, **tag FIBR-0030-complete**. Prior this session (all pushed): **FIBR-0029** ✅ (password hint), **FIBR-0153** ✅ (currency display), **FIBR-0032** ✅ (clipboard copy + auto-clear).
> Tree: about to commit the close + tag + push the FIBR-0030 batch. **NEXT: FIBR-0096** (per-release SHA256SUMS + SBOM — mechanical release tooling) or **FIBR-0154** (3rd-tier category UI). Then an Ants MCP feedback pass at the end of the run.
