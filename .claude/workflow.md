...
| **Active step** | 4→5 (**TDD COMPLETE, gate green** 2026-07-09 — the 12 Deliverables shipped: v6 migration + ambiguity-guarded backfill, `StatementService` + `StatementsWidget` tab, the tabbed `QTabWidget` workspace + Window menu (Center/Reset) + geometry INI outside the vault, `commit_import` reorder+stamp, repos (`add`→int / `id_for_span` / `list_all` / `delete` / `add_batch(stamp)` / `delete_for_statement`), `StatementRow`, `show_done` flag, `home.svg`. 24 new `tests/features/statements/` tests + the FIBR-0051 app_shell ripple (INV-1/2a/4a/6a/9 re-homed to the tab model) + the v5→v6 schema ripple across vault/accounts/categories/import_/ofx_import/pdf_import. `./scripts/ci-local.sh` **322 passed/1 skipped**, mypy 0, ruff/format/bandit/gitleaks clean. Real Standard Bank credit-card PDF validated end-to-end in a **throwaway** vault (53 txns parsed, Statements tab count == inserted == total, delete removed exactly those 53 + the record) — PDF/password never committed. Next: `/close-phase` (audit + indie-review).) |
| **Blocked on** | — |
...
1. ✅ Write + cold-eyes spec (`docs/specs/FIBR-0052.md` — **converged loop 3, 2026-07-09**; 0 CRIT/0 HIGH, design sound; cleared for code)
2. ✅ Verify dependencies on the roadmap DAG (FIBR-0051 ✅; independent of P08/P09)
3. ✅ Write failing tests (`tests/features/statements/` 24 tests + the FIBR-0051 app_shell ripple + the v5→v6 schema ripple across 6 suites)
4. ✅ Implement until tests pass (12 Deliverables: v6 migration+backfill, StatementService/StatementsWidget, tabbed shell + Window menu + geometry, `show_done` flag, `home.svg`; gate green **322 passed/1 skipped**, mypy 0; real Standard Bank PDF validated end-to-end in a throwaway vault — 53 txns, exact count, clean delete)
5. ⬜ Run `/audit`
6. ⬜ Run `/indie-review`
