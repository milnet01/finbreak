...
> **CURRENT (2026-08-02) — v0.1.19 RELEASED + PUBLISHED, and FIBR-0203 (the missing v0.1.17 release) CLOSED first so the notes chain is whole.** Three tabs' worth of work that had been invisible since v0.1.18 (2026-07-28) is now in users' hands: the FIBR-0113 Accounts table, the FIBR-0198 reveal, and FIBR-0192's columns. **Order was deliberate — v0.1.17 back-filled BEFORE cutting v0.1.19**, so a user on 0.1.16 gets the full 0.1.17 → 0.1.18 → 0.1.19 chain in one prompt rather than a hole in the middle. **FIBR-0203 ✅ notes-only** (the open decision in its own bullet; artifacts would have meant signing binaries from a 10-day-old tag that v0.1.18 already superseded and nobody should download). The one real hazard there was `--latest=false`: GitHub picks "latest" by DATE, so a release object created today for an older tag would have become `/releases/latest` — the endpoint `check_for_update` reads to decide what to OFFER. Verified after publishing that it still returned v0.1.18. **The fix was verified against the LIVE API, not by inspection:** real `fetch_releases` + `_accumulated_notes` for 0.1.16→0.1.18 went from 0.1.18's body alone to both `## 0.1.18` and `## 0.1.17` (11,032 chars). No code change, no CHANGELOG entry — the defect was a missing release object, not app behaviour. **v0.1.19 itself:** 11 CHANGELOG entries (4 Added / 4 Changed / 3 Fixed), version lockstep green across all eight version-bearing files, gate green **1455 passed, 2 skipped**. Commits `490f164`(roadmap)→`f4de4c4`(bump)→`a45fdd8`(flatpak pin)→`c2edbef`; tag `v0.1.19` on `f4de4c4`. The **Flatpak `commit:` pin landed as a follow-up commit AFTER the tag** (the v0.1.18 `f8a2ccd` precedent) — the recipe's todo 3 cannot be satisfied before the tag sha exists. Metainfo `<release>` + deb changelog stanza written **by hand** (the recipe suggests the `changelog-writer` agent; the session forbade unrequested agents). **README refreshed per the recipe's judgement todo:** the feature prose was already current from the FIBR-0113 close, but three 0.1.19 changes were absent — dashboard scroll (FIBR-0186), Reset layout (FIBR-0192), single-instance (FIBR-0189) — now added; and the standing sections were **verified against reality rather than trusted**: v0.1.18 genuinely shipped BOTH `.exe` and `.exe.sig`, and FIBR-0133 (SignPath) + FIBR-0159 (Flathub) are both still 🚧, so the "declined" / "still to come" wording stands unchanged. **Release integrity verified on the PUBLISHED bytes, not the local pre-upload copies** (so upload corruption is covered too): AppImage `.sig` ✓, `.exe` `.sig` ✓ (this is what activates the Windows updater), `SHA256SUMS.sig` ✓, both digests match their manifest lines, manifest carries BOTH platforms — all against the committed `RELEASE_PUBLIC_KEY_B64`. `/releases/latest` → **v0.1.19**. 8 assets, identical shape to v0.1.18. Clean-room (Python-free container) passed both platforms. **Filed this session — two pre-existing defects found while releasing, neither a blocker:** **FIBR-0205** (📋 test) — `pytest tests/features/bundling` ALONE SIGABRTs with a 12 MB coredump: `test_INV1_selftest_fail_names_the_broken_stack` stubs `_check_qt` (the function that CONSTRUCTS the QApplication) but leaves `_check_icons` rendering a real pixmap, and Qt's `qFatal` calls `abort()` so `run_self_test`'s `except Exception` cannot catch it; it passes in the full suite only because an earlier file leaks a process-wide QApplication (proven both ways: alone → abort, `theme`+`bundling` → 42 passed; reproduced with the bump stashed). Matters because FIBR-0001 INV-6 + CLAUDE.md both advertise single-file runs as supported. **FIBR-0206** (📋 fix) — `appstreamcli validate` FAILS on the OBS metainfo, 6 × `screenshot-image-not-found`; cause narrowed to a wrong URL **path**, not missing uploads: the images are live at `/assets/img/shots/finbreak-<name>.png` (all six HTTP 200) while the metainfo asks for `/img/finbreak/<name>.png` (404) — and the basename gains a `finbreak-` prefix too, so a path-only sed still 404s. Bears on FIBR-0159 (Flathub CI runs `appstreamcli`). **NEXT (user-agreed order, 2026-08-02):** FIBR-0199 + FIBR-0200 as ONE pass — the fixture FIBR-0199 needs (build a remembered view, destroy it before the reset) is the same scenario FIBR-0200 is about (a `_save` closure outliving its header), so one spec + one TDD cycle; then ONE shared spec for FIBR-0201 + FIBR-0202, since both independently derived the same multi-select design (widen `SelectionMode.SingleSelection`, resolve ticked rows to ids BEFORE mutating, decide the transaction boundary) and specing them apart risks two different answers — FIBR-0202's per-call FIBR-0148 hand-off trap is its own section on top.

> **PRIOR (2026-08-02) — FIBR-0192 SHIPPED + CLOSED by `/close-phase`; its fix-pass FP01 (FIBR-0204) applied.** Window → Reset layout now resets column widths and order as well as window geometry, and the Forecast events table + the three Home breakdown trees joined the shared column scheme — which closes **FIBR-0084** too (both ✅). Built TDD off the accepted spec (converged after 4 cold-eyes loops + a mid-run split); implementation commit `70f0e15`, gate green **1455 passed, 2 skipped**. `remember_columns` widened to `QTableWidget | QTreeWidget` via `_header_of`; the build-time header state captured into a `finbreak_default_header_state` dynamic property; new `reset_columns(root)` sweeping `findChildren`; `_reset_layout` reordered to resize → centre → `reset_columns` → clear → sync and passed `self` (not `_workspace`, which is `None` before unlock). **The close was NOT clean — branch 5a fired.** Static analysis was 0 (semgrep/ruff/bandit) and the test suite came back **strong** (15 mutations, no vacuous leg, all four of §11's "nothing checks this" admissions literally accurate), but two cold review lanes surfaced an actionable list, folded into **FP01 / FIBR-0204** and then applied in the same session. **Two measured Qt facts were wrong and are now corrected in `FIBR-0192-qt-facts.md`:** (1) **QT-10's negative half was read backwards** — `saveState()` *does* serialise `stretchLastSection`, the resize mode, `sectionsClickable`, `defaultSectionSize` and hidden-section flags (new **QT-11**); Probe C's own recorded output showed the captured values winning after a restore, which is the signature of them being *in* the state, and cold-eyes loop 3 had used the misreading to dismiss a lane that was right. The practical rule is now simply "`remember_columns` is the last header call, full stop" rather than an enumeration of two safe-listed flags. (2) **QT-5 was wrong** — `restoreState` emits `sortIndicatorChanged` once per view and the `Qt::SortOrder` marshals cleanly, so `_save` **does** run and `reset_columns` writes + `sync()`s per view (verified against the real functions: the INI's bytes change across the call). Harmless only because `_reset_layout`'s trailing clear erases it. Both source comments that repeated these (`_table_state.py`, `main_window.py:_reset_layout`) were corrected. **Documentation truth:** the item shipped while 15+ places still described it as pending — its own spec's Status line said "ready to implement", `FIBR-0113` told users there was "no escape hatch" and to delete an INI key by hand, and `FIBR-0052` INV-6/INV-6b had never been widened to name the `columns/*` group though FIBR-0192 §12 called it out by name. All corrected; cold-eyes loop-log rows left **frozen** as the record of what each loop believed. Also fixed: the remembered-view count is **13**, not 11 (`recurring.py` and `transfers.py` each build two tables), and INV-6's precondition (b) now asserts a *differing* persisted state rather than mere key existence. **Filed this session:** FIBR-0199 (📋 test — nothing discriminates Reset layout's settings clear), FIBR-0200 (📋 fix — `_save` outliving its header at teardown; the code lane independently confirmed it real, and this change takes the connected closures 8 → 12), FIBR-0201 (📋 bulk-confirm transfers), FIBR-0202 (📋 bulk-delete statements + Delete all), FIBR-0203 (📋 fix — the missing v0.1.17 GitHub **release**, which is not cosmetic: FIBR-0152 builds the in-app update prompt's notes from the release objects, so a user on 0.1.16 offered 0.1.18 sees no 0.1.17 notes at all). **NEXT: user direction.** Nearest: FIBR-0199 / FIBR-0200, then FIBR-0203, then the standing packaging backlog (FIBR-0133 SignPath, FIBR-0159 Flathub). NOTE — the user's Accounts screenshot on 2026-08-02 showed the pre-FIBR-0113 list UI; that is v0.1.18 (released 2026-07-28) and the 5-column Accounts table has been unreleased since 2026-07-30. A release is what surfaces it.

> **PRIOR (2026-07-30) — the Accounts-table chain SHIPPED + PUSHED, all three items (FIBR-0193 → FIBR-0113 → FIBR-0198; 6 commits `678f001`→`14eb621`).** Built by TDD off three already-gated specs (no cold-eyes re-run — the gate closed on 2026-07-30). **FIBR-0193** (storage): schema **v13** adds nullable `accounts.account_number` + `accounts.note`; `models.Account` gains both appended after `created_at` (defaulted, keeping the 4-arg positional literals working); both repo read paths SELECT six columns in field order; `add`/`update` take the two parameters **REQUIRED** (an unconditional `UPDATE … SET` means a defaulted `None` silently erases), while `add_account`'s are optional kwargs — the asymmetry is the point; a new module-level `_normalise_optional` on both service write paths stores a blank as SQL `NULL`. **FIBR-0113** (the table): `QListWidget` → 5-column sortable `QTableWidget` (`accounts_table`, `remember_columns`), a two-row form with the two new fields, and the number masked on BOTH surfaces by two different mechanisms (cell = text substitution via `_mask_account_number`, field = `Password` echo; the form always holds the RAW value). Row identity moved to `_table_state`, and the six `_ACCOUNT_*_ROLE` constants were **deleted as a correctness requirement** — `_ACCOUNT_ID_ROLE` and `_ROW_INDEX_ROLE` are both `Qt.ItemDataRole.UserRole`, so a survivor makes `selected_index` return a database id used as an index. Clear-then-fill (a leading `setRowCount(0)` in `fill_guard`), because the sibling's reuse-rows shape lets a selection ride its item through a re-sort onto a **different account** (reproduced against real Qt; the Statements twin is FIBR-0194, untouched). Status is a `SortableItem` on a severity rank; the four `tr()` literals dropped their `"  ·  "`. **FIBR-0198** (the reveal): a "Show account numbers" checkbox, session-scoped, never persisted, with a parented single-shot 30s `QTimer` (a COPIED literal, not an import of `DEFAULT_CLIPBOARD_CLEAR_SECONDS`); `_on_reveal_toggled` re-selects under `blockSignals`/`finally` so a re-mask cannot clobber a mid-edit; `_refresh()` gained a `VaultLockedError` guard around all five pre-fill reads, since a queued timeout can land between the lock and the deferred `deleteLater()`. **TDD trail (all red first):** `test_migration_v13.py` (5 legs) run RED before the migration existed; INV-3/5/6/7 red in `test_accounts.py`; **19** red before the table rewrite; 5 red before the toggle. **Four existing legs were RE-DERIVED, not re-pointed** — a mechanical `_list`→`_table` swap would have left them passing while checking less, the FIBR-0128 sentinel sweep above all (its literal role list degrades to two roles that could never hold a password while four columns go unswept). **Schema-churn sweep:** 14 test files re-pinned to 13, 6 test functions renamed (the 5 stale `_is_10` names are pre-existing FIBR-0144 evidence and stay), the `v12`-comment sweep across 5 files, 6 feature `spec.md` prose fixes. **Cross-doc, all §12 bullets shipped WITH their item and every pending marker flipped to applied:** FIBR-0005 / FIBR-0013 / FIBR-0128 / FIBR-0177 annotated, ROADMAP FIBR-0086 amended (its storage half landed here at v13, not v8), `tests/features/accounts/spec.md` now carries **five** INV blocks, README + both Accounts screenshots refreshed (the demo seeder gained numbers/notes so the masking is visible), and **`security-model.md` T13 amended + T14 added** — the reveal makes an account number copyable from the form field and that copy is **not** auto-cleared (`ClipboardAutoClear` is wired only to the transactions list), a deliberate gap now stated rather than implied. Gate green **1448 passed, 2 skipped**; three ROADMAP bullets ✅, three CHANGELOG entries. **No tags** — these are three thirds of one feature, not phase closes. **NEXT: user direction.** Open near-term: FIBR-0192 (finishes FIBR-0084 — the Forecast table + Home trees have no column persistence, Reset layout does not clear saved column state), FIBR-0194 (the latent `StatementsWidget.refresh` selection drift this chain reproduced but did not fix), FIBR-0086 (account-number detection + import matching, now unblocked by v13), plus the standing packaging backlog (FIBR-0133 SignPath, FIBR-0159 Flathub, FIBR-0152).

> **CURRENT (2026-07-26) — full `/debt-sweep`, all lanes, all severities: COMPLETE (4 commits, DS02).** Static analysis clean first (`audit_run` full-tree, 7 tools — ruff/bandit/semgrep/trivy/mypy/gitleaks/shellcheck — **0 findings**). The mechanical `debt_sweep_scan` returned 67 test_coverage findings and 0 for the other three categories; **both numbers were wrong**, and verifying that was the sweep's main work. **65 of the 67 were false** — the detector greps the literal `INV-N`, but tests tag the id inside the function name with the hyphen dropped (`test_INV8a_...`, forced by Python identifier rules), in banner comments, or via the ticket id; several specs also relocate an invariant to a sibling suite in prose, and one flagged "INV-5" was a cross-reference to *another* spec's invariant, not a declaration. Logged as **allowlist-002** (+ allowlist-003: ROADMAP headlines record work as *proposed*; judge a ✅ bullet by its Resolved body, so FIBR-0068/0069 are not drift). The **2 genuine gaps are now closed by real tests**: `dialog_lifecycle` INV-4 (nothing proved `show_modal` frees a dialog on a NORMAL close — every `isValid` assertion in the suite was lock-driven) and `recurring` INV-2 (determinism/D10 order, shuffled input, tiebreak genuinely exercised). Meanwhile the three "clean" categories held everything actionable, found by 3 manual lanes: **HIGH** — 20 `# noqa` directives cite rules the ruff select list never enabled, so they read as reviewed suppressions while suppressing nothing (removed; rationale kept as prose; **RUF100 added to `select`** so they cannot return; bandit's `# nosec` untouched and still exit 0). **HIGH** — `pyinstaller==6.21.0` hard-coded in 5 build paths + `pip-audit` in windows-build.yml with no lockstep gate (new **bundling INV-7**, scans `git ls-files` so new call-sites are auto-covered; negative-tested). Plus new **flatpak INV-10** (the version-bearing `tag:` was absent from the bump recipe and INV-4 only rejected *bare* semvers), README corrected (it claimed native deb + Flatpak installs that do not exist; SignPath **declined** 2026-07-16, now stated), mypy added to the gate docs in 4 places (the "green pytest isn't a green gate" trap), FIBR-0150 flipped ✅, and ~14 stale docstrings fixed. **NOTE (correction made mid-sweep):** a first pass used `ruff --select RUF100`, which *replaces* the configured rule set — that made every `# noqa: E402` look unused and produced a 59-site over-count. Reverted; `--extend-select` gave the true 20. Deferred + filed: **FIBR-0180** (bookworm→trixie is a glibc-floor decision, not a version-chase), **FIBR-0181** (5 duplicate Decimal→minor conversions — money code, wants its own reproduce-first cycle), **FIBR-0182** (4 dead-code sites, surfaced not deleted per rule 11), **FIBR-0183** (31 bandit "Test in comment" warnings, pre-existing). Ants MCP feedback written (ANTS-3619 marked ✅ complete; 3 new findings + 1 positive). Gate green. **NEXT: user direction.** Open near-term unchanged: FIBR-0178, FIBR-0157, and the packaging backlog (FIBR-0133 SignPath, FIBR-0159 Flathub, FIBR-0152).

> **CURRENT (2026-07-25) — FIBR-0179 forecast-anchor debt-sign fix SHIPPED + PUSHED (commit 9d3355e; user delegated the pick).** The bug filed by FIBR-0177's cold-eyes was **verified first**, not assumed: two RED tests reproduced both halves — a `credit_card` account with a persisted closing balance (R1 200 **owed**) was anchored as **+R1 200 of cash**, and a debt-only vault read `ANCHORED` instead of `NET_FLOW`. Root cause confirmed at source: a debt product's persisted `closing_balance_minor` is the **printed owed figure** (`standard_bank._verify_checksum` Family C reconciles `opening − Σ`, i.e. printed convention = +purchase), the OPPOSITE sign to canonical `amount_minor`, while `ForecastService._anchor` summed `latest_closing_balances()` across ALL types with a plain `balance + roll`. Fix: a new public `CASH_TYPES = {CURRENT, SAVINGS}` gate in `services/forecast.py` — the same set `reconciliation.py` gates on for the same reason (FIBR-0177 D1), deliberately **not** extracted to a shared constant (2 call-sites; extracting would drag a FIBR-0177 spec amendment — Rule of Three). Debt/investment/other now contribute nothing; a debt-only vault falls back to NET_FLOW. Follow-on caught in the same change: the Forecast tab's provenance line said "Excluded (**no recorded balance yet**)" for a card that *does* have a balance — `_excluded_names` now returns a `(no_balance, not_cash)` split with a second, accurate clause. Contract `tests/features/forecast/spec.md` **INV-14** (3 tests: 2 service + 1 qtbot); `docs/specs/FIBR-0171.md` §D1 carries an **amendment block** rather than a new spec doc (a `Kind: fix`, diagnosis already complete in the roadmap bullet — no `/cold-eyes` run; flagged to the user as a judgement call). Gate green **1363 passed, 2 skipped, "All gates passed"**. ROADMAP FIBR-0179 ✅; CHANGELOG Fixed. No tag (not a phase close). **Blast radius verified contained:** `latest_closing_balances()` has exactly one consumer (`ForecastService._anchor`). **NEXT: user direction.** Open near-term: FIBR-0178 (forecast v2 follow-ups — manual balance override, per-account/what-if, CSV balance column), FIBR-0157 (guided first-run wizard), plus the standing packaging backlog (FIBR-0133 SignPath, FIBR-0159 Flathub, FIBR-0152 accumulated update notes).

> **CURRENT (2026-07-24) — FIBR-0172 spending alerts SHIPPED + CLOSED (self-directed autonomous run; user asked to continue FIBR-0172 + FIBR-0177 — BOTH now shipped).** A quiet, dismissable **alerts card** on the Home dashboard (above the selectors, hidden when empty) flags three changes: **(a) new recurring charge** (a suggested OUT stream with `occurrences ≤ 4` — cadence-agnostic "just detected"), **(b) category spike** (a category whose last **complete** month ≥ 2× its prior-3-month integer average; confirmed-transfer + uncategorised rows excluded so totals match the donut), **(c) missed expected debit** (a confirmed OUT stream overdue past `next_expected + 3d`). Pure integer-only detectors + `AlertService` (the sole `Decimal→minor` lives in input prep, keeping detectors `Decimal`-free); new **v12** `alert_dismissals` table (`recurring_decisions` idiom) with per-scope keys (new-recurring permanent / spike per-month / missed-debit per-occurrence); `VaultLockedError`-silent dismiss with a **card-local** rebuild. Spec `docs/specs/FIBR-0172.md` **/cold-eyes CONVERGED loop 3** (2 lanes/loop — caught a **monthly-detector-dead boundary hole** in the first date-window design → reworked to the occurrences gate; a `snapshot(today).confirmed` AttributeError; an INV-15 `Decimal` leak in the (a)/(c) paths; and a **cross-feature v12 drift-guard conflict** with FIBR-0177's `12 not in _MIGRATIONS`). Reproduce-first TDD `tests/features/spending_alerts/` (33 tests, INV-1..19) via an implementation agent; hand-verified the money core + spike window; `/audit` semgrep **0** + cold code-review **0 defects** (folded 1 service-level test-strength add); gate green **1360 passed, 2 skipped**. Schema **11→12** rippled 12 feature guard files + **reworked** the FIBR-0177 reconciliation guard (rename, re-aim at `LATEST+1`, docstring + spec prose). ROADMAP FIBR-0172 ✅; CHANGELOG Added; journal `docs/journal/FIBR-0172.md`; tag `FIBR-0172-complete`. **NEXT: the two delegated items (FIBR-0172 + FIBR-0177) are DONE.** Open near-term: **FIBR-0179** (planned fix — forecast anchor debt-account sign, filed this run, verify-first); FIBR-0178 (forecast v2 follow-ups); plus the standing packaging/i18n backlog. Await user direction for the next pick.
>
> **CURRENT (2026-07-24) — FIBR-0177 account-level reconciliation SHIPPED + CLOSED (self-directed autonomous run; user asked to continue FIBR-0172 + FIBR-0177).** A per-account marker on the **Accounts** tab (`·  ✓ balances reconcile` / `·  ⚠ off by …` / `·  ⚠ N periods don't reconcile`) confirms imported transactions bridge each statement's persisted **closing balance** to the next: for `current`/`savings` accounts, `C_prev + sum_after(P_prev,P_curr] == C_curr` (exact integer minor units, no epsilon) over the consecutive-closing chain. Debt (credit_card/home_loan/personal_loan) + investment + other are quietly `NOT_SUPPORTED` (deferred v2 — their persisted closing is stored in **owed** convention, the opposite sign to canonical `amount_minor`; investment moves with the market). **No schema change** (reuses FIBR-0171 v11 `closing_balance_minor` + `sum_after`). Pure `reconcile_account` core + `ReconciliationService.account_statuses()` (total map) + new repo `closing_balances_for_account`. Spec `docs/specs/FIBR-0177.md` **/cold-eyes CONVERGED loop 3** (2 lanes/loop; correctness lane verified the asset/debt sign argument twice; loop-1 HIGH fixed a wrong `self._auth.vault` citation; filed **FIBR-0179** for the D9 forecast-anchor debt-sign bug). Reproduce-first TDD `tests/features/reconciliation/` (20 tests, INV-1..12) via an implementation agent; hand-verified the money core + repo SQL; `/audit` semgrep **0** + cold code-review **0 defects**; gate green **1327 passed, 2 skipped**. ROADMAP FIBR-0177 ✅; CHANGELOG Added; journal `docs/journal/FIBR-0177.md`; tag `FIBR-0177-complete`. **NEXT: FIBR-0172 spending alerts** — spec **drafted + committed** (`docs/specs/FIBR-0172.md`, pre-cold-eyes: 3 dismissable dashboard alerts (new recurring OUT charge / category spike / missed expected debit), a new **v12** `alert_dismissals` table, pure detectors + `AlertService`); next step is `/cold-eyes --max-loops 7` → TDD → close. Also drafted: FIBR-0179 (planned fix, forecast anchor).
>
> **CURRENT (2026-07-24) — FIBR-0171 cash-flow forecast SHIPPED (self-directed; user delegated the pick + asked to run autonomously).** A new **Forecast tab** projects the balance forward from **confirmed** recurring income/bills (reuses FIBR-0142). The starting number is a **real, current balance**: the closing balance each import already parses is now **persisted** (schema **v11**, `statement_periods.closing_balance_minor`) and **brought current to today** by adding the account's post-statement transactions (`sum_after` over the half-open `(period_end, today]` window), then summed per account; when no statement has a balance yet it falls back to honest **net-flow** framing (line from 0). Pure integer-minor `project_forecast` core (disjoint `(today, horizon]` projection, reuses `_add_cadence`, no Decimal); `ForecastService`; themed line chart + upcoming-events table + horizon picker (End-of-month/30/60/90). Spec `docs/specs/FIBR-0171.md` **/cold-eyes CONVERGED loop 4** (2 lanes/loop; H1 re-architected the mixed-date stale anchor into the brought-current model; migration off-by-one caught v10→**v11**; drift-guard sweep; boundary double-count closed). TDD via an implementation agent (8 committed slices) + my hand-review of the money-critical core + a bounded cold code-review (0 defects; added the missing on-`today` double-count falsifier). Gate green **1307 passed, 2 skipped, "All gates passed"**. ROADMAP FIBR-0171 ✅; CHANGELOG Added; **FIBR-0178** filed for the D12 v2 follow-ups (manual balance override, per-account/what-if, multi-currency, CSV balance column). **NEXT: push + tag `FIBR-0171-complete`; then user direction** (the FIBR-0172 spending-alerts / FIBR-0174 tax-summary / FIBR-0177 reconciliation batch remain planned).
>
> **CURRENT (2026-07-21, autonomous "build it all" run) — trust this + ROADMAP over the stale table below.**
> Shipped + **pushed + tagged** this session (all on origin): **FIBR-0030** ✅ (forgotten-password destructive "start over" reset — `AuthService.reset_vault()` deletes the complete on-disk footprint incl. the SQLite WAL sidecars vault.db-wal/-shm; `StartOverDialog` type-`DELETE` gate; `security-model.md` INV-12; spec cold-eyes CONVERGED loop 6; gate green 1209; tag FIBR-0030-complete, commit d891206), plus **FIBR-0029** (password hint), **FIBR-0153** (currency display), **FIBR-0032** (clipboard copy + auto-clear).
> **SHIPPED + pushed + tagged this session too — FIBR-0096** ✅ (per-release signed `SHA256SUMS` + per-platform CycloneDX SBOM). Spec cold-eyes ran **8 loops** (security/cross-ref lane clean 6–8; release-shell hardened; user chose TDD at the cap). Ships `scripts/gen-checksums.sh` (merge-aware manifest helper), a **two-phase** signed manifest (`clean → release-view-guided fetch → verify-fetched → merge → sign → hard-gate verify → publish`) with an **anti-laundering fetch-verify gate** + fail-closed fetch, a per-platform SBOM (freeze-before-PyInstaller `pip-audit -r --no-deps`), and `security-model.md` INV-13. Reproduce-first TDD: `tests/features/release_integrity/` (10 tests, INV-1..7). Gate green **1219 passed**; tag `FIBR-0096-complete`, commit eeddd38.
> **IN PROGRESS — FIBR-0154** (3rd-tier category UI, hard-capped at 3; user decision 2026-07-21 = **exactly 3 levels**). Data model + service already allow arbitrary depth (FIBR-0006); gap is **UI-only**. **Spec `docs/specs/FIBR-0154.md` DRAFTED + cold-eyes ran the full 7 loops** (service/data lane clean 3×; core design confirmed sound — no depth-cap hole, wrong-parent trap closed; the tail was interaction-completeness on the redesign, all fixed). Key design: **Add anchored to the tree selection** (child of the selected node; disabled on Level-3/empty — the creation cap); **Update = rename and/or re-parent** (`update_category(id, name.strip() or current, move_under.currentData())`, empty name ⇒ keep current so pure re-parent works; Types → Update/Delete disabled); a **subject-aware "Move under…"** re-parent selector (childed L2 → Types only); breadcrumb picker labels via a NEW `sub_category_parent_names()` map + optional `parent_names` on `add_grouped_categories` (grouped shape UNCHANGED so transactions.py + tests survive); §7 lists ALL test migrations (conftest stubs, local `_stub_rule_dialog`, rewrite `test_INV7b`/`test_INV7cd`/`test_add_category_swallows`; `test_INV7f` RETAINED). **Spec CONVERGED — cold-eyes loop 8 (verify) clean on both cold lanes, zero verified findings (spec §8); signed off + pushed (commit 1f3385d).**
> **✅ SHIPPED + tagged 2026-07-21 (commit 35131cd, tag FIBR-0154-complete).** Reproduce-first TDD: 17 RED INV-1..7 (baseline 1505986) → implemented 6 source files → **gate green 1235 passed, 2 skipped ("All gates passed")**. ROADMAP flipped ✅, CHANGELOG Added entry. **TDD trail (historical):** RED baseline was 17 failing tests + 85 pass, no collateral breakage. Files: `tests/features/categories/test_categories.py` (INV-1/2/3/5/7 + rewritten swallow test; `test_INV7f` retained; deleted `test_INV7b`/`test_INV7cd`, intent migrated), NEW `tests/features/categories/test_picker_labels.py` (INV-4 accessor + 2 dialog-render + 4 capture-spy + filter-combo legs), `tests/conftest.py` (`_CallLog` list-subclass capturing `parent_names`; `PickerStub`/`RuleStub`/`stub_picker`/`spy_learning` accept+capture it), `tests/features/categorisation/test_categorisation.py` (local `_stub_rule_dialog` accepts `parent_names`).
> **✅ SHIPPED + tagged 2026-07-21 — FIBR-0156** (commit 128fc55, tag FIBR-0156-complete). A single top-level "Report an Issue" menu-bar action right of Donate → opens `REPORT_ISSUE_URL` (`https://github.com/milnet01/finbreak/issues/new`) via `_open_url` (browser egress, not an app fetch — security-model INV-8; mirrors Donate). Chose single-action (no submenu, no version/OS pre-fill) per shortest-correct; pre-fill = easy follow-up. Reproduce-first TDD: INV-8b RED (AttributeError, feature-absent) → 3 edits in `main_window.py` (constant + action + `menu.addAction`) → **gate green 1236 passed, 2 skipped**. `action_report_issue` added to INV-1 canonical set; spec.md INV-8b row + menu-bar enumeration updated.
> **✅ SHIPPED + tagged 2026-07-21 — FIBR-0155** (OBS native RPM/deb packaging). Spec `docs/specs/FIBR-0155.md` cold-eyes-CONVERGED loop 6 (74 findings across 6 loops). **Native-from-source** decision: a PyInstaller `--onedir` frozen runtime under `/usr/lib/finbreak/` — the security-critical stack (SQLCipher/qpdf/pdfium/Qt) stays **bundled**, the exact pinned closure the gate tests, not distro-shared (§ 3.2). Four confirmed targets (Tumbleweed, Fedora, Debian 13, Ubuntu 24.04) **+ Leap 15.6 pending one glibc check** (§ 5 — likely a 5th; reverses the earlier draft's drop). Ships `packaging/obs/` (finbreak.spec [dual `%if suse/fedora`], `debian/` recipe, `io.github.milnet01.finbreak.desktop` + `.metainfo.xml` [validates via `appstreamcli`], `finbreak.sh` launcher, `_service`, README runbook). Edits: pyproject `[project.scripts]`, `app.py` Wayland app_id → reverse-DNS app-ID (X11 `WM_CLASS` stays `finbreak`), security-model INV-8 distro note, naming.md app-ID resolved, `bump.json` +2 sync todos + `post_check`. Reproduce-first TDD `tests/features/obs_packaging/` INV-1..8 (RED pre-recipes → green). Gate green **1244 passed, 2 skipped**. **CAVEAT:** the "real OBS build green" exit criterion is a **manual maintainer step** on the user's build.opensuse.org account (documented `osc` flow + the § 5 pre-submit checklist in `packaging/obs/README.md`: Leap glibc go/no-go, per-distro package names, wheel vendoring, live `xprop WM_CLASS`, screenshots) — like FIBR-0131's live-swap caveat, not automatable in this repo's CI. **NEXT STEP — the Ants MCP feedback pass** to close the "build it all" run (mark completed fixes in `/mnt/Games/Scripts/Linux/finbreak_Ants_MCP_Feedback.md`, don't overwrite maintainer roadmap-ID annotations). **The FIBR-0154 API surface delivered (for reference):**
>  • `ui/categories.py` `CategoriesWidget`: `_tree` renders **recursively** depth 3 (nested Type→Category→Sub-category); `_select_category(id)` **recurses to depth 3**; `_name` **starts empty** on selection (current name = placeholder only, no `setText` auto-load); NEW `_move_under: QComboBox` (**replaces `_type`**) — subject-aware re-parent, **Update only**, ids as userData, rebuilt in `_on_selection_changed` preselecting subject's parent, **childed L2 → Types only**, **childless L2 → {Types + other L2} minus itself**; `_add_button.isEnabled()` = cap (disabled when nothing selected or selection Level-3+, enabled L1/L2); `_on_add` = `add_category(selected.id, name)`; `_on_update` = `update_category(selected.id, name.strip() or selected.name, _move_under.currentData())`; retained `_update_button`/`_delete_button`/`_error`/`_kind_of_item`; drop D9 docstring; drop orphaned `select_combo_data` import if unused (ruff F401).
>  • `services/categorization.py`: NEW `sub_category_parent_names() -> dict[int,str]` (grandchild-or-deeper id → immediate-parent name; **Level-2 ids ABSENT**). `leaf_categories_grouped()` shape UNCHANGED.
>  • `ui/_widgets.py`: `add_grouped_categories(combo, grouped, parent_names=None)` renders L3 rows as translatable `"{parent} › {row} ({type})"` (U+203A `›`, via `translate(_LABEL_CONTEXT,…)`; default None = today).
>  • `ui/category_picker.py` / `ui/rules.py`: `CategoryPickerDialog.__init__(grouped, current_category_id, parent=None, parent_names=None)` and `RuleEditDialog.__init__(grouped, pattern="", category_id=None, parent=None, parent_names=None)` — `parent_names` **appended LAST after `parent`** (positional-safe), forwarded to internal `add_grouped_categories`.
>  • `ui/transactions.py`: thread `sub_category_parent_names()` at filter combo (`:255`), Set-category (`:403`), learn-offer (`:420`); `ui/rules.py` at Add (`:196`) + Edit (`:221`). Grouped `c.id` intersection unchanged.
> **User chose "finish FIBR-0154 first"** (2026-07-21) over the two freshly-roadmapped requests. **AFTER FIBR-0154:** **FIBR-0156** (planned — small: a "Report an Issue" menu item to the RIGHT of Donate opening the GitHub issues URL via `self._open_url`, mirroring Donate `main_window.py:360-376`), then **FIBR-0155** (planned — OBS/openSUSE Build Service native RPM/deb publishing; needs a spec + native-vs-AppImage decision), then the **Ants MCP feedback pass** (mark completed fixes in `/mnt/Games/Scripts/Linux/finbreak_Ants_MCP_Feedback.md`, don't overwrite maintainer roadmap-ID annotations) to close the run.
