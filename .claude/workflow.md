...
| **Next gate** | **The 2026-07-09 user-facing queue is CLOSED — all four shipped this session** (FIBR-0057 import-account fix ✅, FIBR-0060 Wayland window-size + KWin Center ✅, FIBR-0058 ofxparse-warning/bs4-pin ✅, FIBR-0059 edit-statement-account ✅; tags `*-complete`, pushed through `af3aac1`). Near-term chore **FIBR-0061 CLOSED 2026-07-09** (commit `383ee92`): mypy is now a gate stage in `ci-local.sh` (so CI + the pre-push hook enforce it) and the 4 pre-existing test-tree type errors are fixed. Remaining phase-ordered work: **FIBR-0010** (P08 rules engine — the active item) → FIBR-0054 auto-update → P09/FIBR-0011 → P10/FIBR-0012. **User note:** with FIBR-0057 shipped, importing into the wrong account is now correctable on the preview step (and FIBR-0059 fixes any already-logged mis-link via the Statements tab "Change account" button) — the re-import order no longer matters. Center Window + window-size now work on the user's KDE Wayland (live-verified). |
