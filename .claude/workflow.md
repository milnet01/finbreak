...
> Just shipped (local, unpushed): **FIBR-0032** ✅ (clipboard "Copy amount"/"Copy description" + auto-clear) — new `ClipboardAutoClear(QObject)` helper (single owned single-shot QTimer wired directly to `clear_if_ours`, live per-copy timeout, clears only if the clipboard still holds our value); `AuthService.clipboard_clear_seconds` getter/setter, `ALLOWED=(10,30,60,0)`, `DEFAULT=30`, `0`="Never"; Settings combo; PDF password + account numbers stay NON-copyable (FIBR-0128 INV-1 preserved). Spec `docs/specs/FIBR-0032.md` cold-eyes converged loop 9; `security-model.md` T13 cold-eyes converged loop 1 (polish). 23 clipboard tests; full gate green (1164 passed, 2 skipped). commit 0b45573 (impl) + 7d93443 (doc polish), **tag FIBR-0032-complete**. Also filed this session: **FIBR-0153** 📋 (currency display: symbol + separate Currency column), **FIBR-0154** 📋 (3rd-tier category UI). Earlier this session: **FIBR-0151** + **FIBR-0033** (pushed).
> **UNPUSHED BATCH queued** — cold-eyes spec loops (FIBR-0032 loops 1–9) + impl + doc polish + FIBR-0153/0154 filings + this close-out. Push the batch next. **NEXT after push: FIBR-0153** (currency display) — needs its own spec → /cold-eyes --max-loops 7 → TDD; or FIBR-0029/0030/0096 from the backlog.
