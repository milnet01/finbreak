...
> **TDD IN PROGRESS (as of 2026-07-21):** reproduce-first RED baseline COMMITTED LOCALLY (commit 1505986, NOT pushed — gate correctly reds on failing tests until the feature lands). **17 failing tests** (INV-1..7) + 85 pass, no collateral breakage — confirmed by the driver. Files: `tests/features/categories/test_categories.py` (INV-1/2/3/5/7 + rewritten swallow test; `test_INV7f` retained; deleted `test_INV7b`/`test_INV7cd`, intent migrated), NEW `tests/features/categories/test_picker_labels.py` (INV-4 accessor + 2 dialog-render + 4 capture-spy + filter-combo legs), `tests/conftest.py` (`_CallLog` list-subclass capturing `parent_names`; `PickerStub`/`RuleStub`/`stub_picker`/`spy_learning` accept+capture it), `tests/features/categorisation/test_categorisation.py` (local `_stub_rule_dialog` accepts `parent_names`).
> **NEXT STEP — IMPLEMENT the 6 source files to the RED tests' API contract (tests are authoritative), then driver runs `./scripts/ci-local.sh` → green → close (flip ROADMAP ✅, CHANGELOG, tag `FIBR-0154-complete`, push batch).** The API surface the tests DEFINE (implementer must match exactly):
